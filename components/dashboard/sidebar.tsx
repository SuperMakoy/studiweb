...
function FileTreeItem({ file, active }: { file: RecentFile; active: boolean }) {
  ...
  const extColor: Record<string, string> = {
    txt: "#7f9fff", doc: "#5dade2", docx: "#5dade2", pdf: "#f1948a",
  }
  ...
  return (
    <Link
      ...
      style={{
        ...
        padding: "5px 8px 5px 20px", borderRadius: 7,
        background: active ? "rgba(91,110,232,0.14)" : hovered ? "rgba(255,255,255,0.04)" : "transparent",
        border: "1px solid", borderColor: active ? "rgba(91,110,232,0.22)" : "transparent",
    ...
    >
      {/* Connector tick */}
      <div style={{ position: "absolute", left: 9, top: "50%", width: 7, height: 1, background: "rgba(255,255,255,0.1)", transform: "translateY(-50%)" }} />

      {/* File type dot */}
      <div style={{ width: 5, height: 5, borderRadius: "50%", background: dotColor, flexShrink: 0, opacity: isHighlighted ? 1 : 0.5, transition: "opacity .15s" }} />

      {/* File icon */}
      <svg width="11" height="11" viewBox="0 0 12 12" fill="none" style={{ flexShrink: 0, opacity: isHighlighted ? 0.7 : 0.3, transition: "opacity .15s" }}>
        <rect x="1.5" y="1" width="9" height="10" rx="1.5" stroke="rgba(255,255,255,0.8)" strokeWidth="1" />
        <path d="M3.5 4.5h5M3.5 6.5h5M3.5 8.5h3" stroke="rgba(255,255,255,0.8)" strokeWidth="0.9" strokeLinecap="round" />
      </svg>

      {/* File name */}
      <span style={{
        fontSize: 11, fontWeight: active ? 600 : 400,
        color: active ? "#b8c4ff" : hovered ? "rgba(255,255,255,0.65)" : "rgba(255,255,255,0.35)",
        whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", flex: 1, transition: "color .15s",
      }}>
        {name}
      </span>
    </Link>
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    getQuizHistory()
      .then((history) => setQuizCount(history.length))
      .catch(() => setQuizCount(0))
  }, [])

  const fileLibActive = pathname === "/file-library" || pathname.startsWith("/file-library/")
  const activeFileId = pathname.startsWith("/file-library/") ? pathname.split("/file-library/")[1] : null
  ...
  // Derive display name and avatar initial from real auth user
  ...
  // Role label — basic for now, could be extended
  ...
  return (
    <>
      <style>{`
        ...
        .studi-nav-link:hover { background: rgba(255,255,255,0.05) !important; color: rgba(255,255,255,0.85) !important; }
        .studi-filelib-link:hover { background: rgba(255,255,255,0.05) !important; color: rgba(255,255,255,0.85) !important; }

        .file-tree-scroll { overflow-y: auto; overflow-x: hidden; scrollbar-width: none; -ms-overflow-style: none; }
        .file-tree-scroll::-webkit-scrollbar { display: none; }

        .file-tree-wrap { position: relative; }
        ...
        @keyframes fileSlideIn { from{opacity:0;transform:translateX(-5px)} to{opacity:1;transform:translateX(0)} }
        .file-tree-item { animation: fileSlideIn .14s ease forwards; opacity: 0; }
        ...
        .sidebar-logout:hover { background: rgba(255,107,107,0.08) !important; color: rgba(255,107,107,0.9) !important; }
      `}</style>

      <aside
        className="studi-sidebar hidden md:flex flex-col flex-shrink-0 h-screen sticky top-0"
        style={{ width: 200, background: "#0d0d2b", borderRight: "1px solid rgba(255,255,255,0.06)" }}
      >
        {/* ── Logo ── */}
        ...
        {/* ── Menu label ── */}
        <div style={{ padding: "0 20px 6px", fontSize: 10, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "1.5px", fontWeight: 600, flexShrink: 0 }}>
          Menu
        </div>
        ...
        <nav style={{ padding: "0 8px", flexShrink: 0 }}>
          ...
          <Link
            ...
            style={{
              ...
              border: pathname === "/dashboard" ? "1px solid rgba(91,110,232,0.25)" : "1px solid transparent",
              textDecoration: "none", marginBottom: 2, transition: "all .2s",
            }}
          >
          ...
          <div style={{ marginBottom: 2 }}>
            <div style={{ display: "flex", alignItems: "center" }}>
              <Link
                ...
                style={{
                  ...
                  border: fileLibActive ? "1px solid rgba(91,110,232,0.25)" : "1px solid transparent",
              ...
              {!loadingFiles && allFiles.length > 0 && (
                <button
                  ...
                  style={{ padding: "6px 5px", background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", borderRadius: 6, transition: "color .15s", flexShrink: 0 }}
                  title={filesOpen ? "Collapse" : "Expand"}
                >
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ transform: filesOpen ? "rotate(0deg)" : "rotate(-90deg)", transition: "transform .2s ease" }}>
            ...
            {/* File tree */}
            {filesOpen && (
              <div className="file-tree-wrap" style={{ marginLeft: 19, borderLeft: "1px solid rgba(255,255,255,0.07)", marginTop: 2 }}>
                <div className="file-tree-scroll" style={{ maxHeight: 180, paddingBottom: 20, display: "flex", flexDirection: "column", gap: 1 }}>
                  {loadingFiles ? (
                    [78, 92, 58, 85].map((w, i) => (
                      <div key={i} className="skel" style={{ height: 24, borderRadius: 6, background: "rgba(255,255,255,0.06)", margin: "1px 6px 1px 10px", width: `${w}%` }} />
                    ))
                  ) : allFiles.length === 0 ? (
                    <div style={{ padding: "5px 8px 5px 18px", fontSize: 11, color: "rgba(255,255,255,0.18)", fontStyle: "italic" }}>
                      No files yet
                    </div>
                  ) : (
                    allFiles.map((file) => (
                      <div key={file.id} className="file-tree-item">
                        <FileTreeItem file={file} active={activeFileId === file.id} />
                      </div>
                    ))
                  )}
                </div>
          ...
          <Link
            ...
            style={{
              ...
              border: pathname === "/study-ai" ? "1px solid rgba(91,110,232,0.25)" : "1px solid transparent",
              textDecoration: "none", marginBottom: 2, transition: "all .2s",
            }}
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#7f9fff" }}>
                  {allFiles.length}
                </div>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: ".8px", marginTop: 1 }}>Files</div>
              </div>
              <div style={{ width: 1, background: "rgba(255,255,255,0.06)" }} />
              <div style={{ textAlign: "center" }}>
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, color: "#58d68d" }}>
                  {quizCount === null ? "—" : quizCount}
                </div>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: ".8px", marginTop: 1 }}>Quizzes</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginTop: "auto", padding: "16px 8px", flexShrink: 0 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 12, fontWeight: 600, color: "rgba(255,107,107,0.8)",
              background: "none", border: "none", cursor: "pointer",
              display: "flex", alignItems: "center", gap: 8, transition: "all .2s", fontFamily: "inherit",
            }}
          >
