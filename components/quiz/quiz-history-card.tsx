  ...
  return (
    <>
      <div
        ...
        className="bg-[#5B6EE8] text-white rounded-xl md:rounded-2xl p-3 md:p-5 hover:shadow-xl transition transform hover:scale-105 cursor-pointer"
      >
        <div className="flex justify-between items-start mb-3 md:mb-4">
          <h3 className="text-sm md:text-lg font-bold">Quiz Details</h3>
        </div>

        {showFileName && (
          <div className="mb-3 md:mb-4">
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">File Name</p>
            <p className="text-xs font-semibold truncate">{quiz.displayName || quiz.fileName}</p>
          </div>
        )}

        <div className="grid grid-cols-2 gap-3 md:gap-4 mb-3 md:mb-4">
          <div>
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">Score</p>
            <p className="text-xl md:text-2xl font-bold">
              {quiz.score}/{quiz.totalQuestions}
            </p>
            <p className="text-sm md:text-lg opacity-90 mt-0.5">{percentage}%</p>
          </div>

          <div>
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">Points Earned</p>
            <p className="text-xl md:text-2xl font-bold">{quiz.points || 0}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <div>
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">Time Taken</p>
            <p className="text-sm md:text-base font-semibold">{quiz.timeElapsed}</p>
          </div>

          <div>
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">Difficulty</p>
            <p className="text-sm md:text-base font-semibold">{formatDifficulty(quiz.difficulty || "moderate")}</p>
          </div>
        </div>

        {quiz.completedAt && (
          <div className="mt-3 md:mt-4 pt-3 md:pt-4 border-t border-white border-opacity-20">
            <p className="text-[10px] opacity-70 uppercase tracking-wide mb-0.5">Completed On</p>
            <p className="text-xs">
